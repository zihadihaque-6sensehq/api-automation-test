// Auto-generated post-request tests for TC_011
...
pw.test("TC_011: Wrong password - HTTP status", function () {
  var allowed = [400, 401, 403];
  var ok = allowed.indexOf(pw.response.status) !== -1;
  pw.expect(ok).toBe(true);
});

pw.test("TC_011: Wrong password - no access token", function () {
  var token = responseToken(responseJson());
  var ok = token === null || token === undefined || token === '';
  pw.expect(ok).toBe(true);
});

pw.test("TC_011: Wrong password - response time under 30s", function () {
