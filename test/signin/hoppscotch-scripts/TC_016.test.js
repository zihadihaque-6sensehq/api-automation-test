// Auto-generated post-request tests for TC_016
...
pw.test("TC_016: API success with token - HTTP status", function () {
  var allowed = [200, 201];
  var ok = allowed.indexOf(pw.response.status) !== -1;
  pw.expect(ok).toBe(true);
});

pw.test("TC_016: API success with token - access token present", function () {
  var token = responseToken(responseJson());
  var ok = typeof token === 'string' && token.trim().length > 0;
  pw.expect(ok).toBe(true);
});

pw.test("TC_016: API success with token - response time under 30s", function () {
