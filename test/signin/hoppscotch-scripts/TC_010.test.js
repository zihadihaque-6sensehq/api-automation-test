// Auto-generated post-request tests for TC_010
...
pw.test("TC_010: Sign in valid - HTTP status", function () {
  var allowed = [200, 201];
  var ok = allowed.indexOf(pw.response.status) !== -1;
  pw.expect(ok).toBe(true);
});

pw.test("TC_010: Sign in valid - access token present", function () {
  var token = responseToken(responseJson());
  var ok = typeof token === 'string' && token.trim().length > 0;
  pw.expect(ok).toBe(true);
});

pw.test("TC_010: Sign in valid - response time under 30s", function () {
