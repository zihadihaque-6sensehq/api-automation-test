// Auto-generated post-request tests for TC_017
...
pw.test("TC_017: Invalid email format - HTTP status", function () {
  var allowed = [400, 422];
  var ok = allowed.indexOf(pw.response.status) !== -1;
  pw.expect(ok).toBe(true);
});

pw.test("TC_017: Invalid email format - no access token", function () {
  var token = responseToken(responseJson());
  var ok = token === null || token === undefined || token === '';
  pw.expect(ok).toBe(true);
});

pw.test("TC_017: Invalid email format - error message", function () {
  var msg = responseError(responseJson()).toLowerCase();
  var ok = /email/i.test(msg);
  pw.expect(ok).toBe(true);
});

pw.test("TC_017: Invalid email format - response time under 30s", function () {
