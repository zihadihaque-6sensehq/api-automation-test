// Auto-generated post-request tests for TC_014
...
pw.test("TC_014: Failed login error - HTTP status", function () {
  var allowed = [400, 401, 403];
  var ok = allowed.indexOf(pw.response.status) !== -1;
  pw.expect(ok).toBe(true);
});

pw.test("TC_014: Failed login error - no access token", function () {
  var token = responseToken(responseJson());
  var ok = token === null || token === undefined || token === '';
  pw.expect(ok).toBe(true);
});

pw.test("TC_014: Failed login error - response time under 30s", function () {
